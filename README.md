...
Chart.js uses the **strategy** design pattern to create line segments. The line component is composed of two line segmentation strategies: [(regular) pathSegment](https://github.com/chartjs/Chart.js/blob/6a6693e26dc190acff9426174256bfe730f09629/src/elements/element.line.js#L68) and [fastPathSegment](https://github.com/chartjs/Chart.js/blob/6a6693e26dc190acff9426174256bfe730f09629/src/elements/element.line.js#L115). The interface [_getSegmentMethod](https://github.com/chartjs/Chart.js/blob/6a6693e26dc190acff9426174256bfe730f09629/src/elements/element.line.js#L182) decides which method to use based on a few criteria, such as if the line is decimated. The chosen method is then used as the segmentation method property when creating a line object. Regardless of the method chosen, the segmentation method will take in the parameters ctx, line, segment, and params to create the segment.
...
Another OOP design principle that is important and one of the main ideas of object-oriented programming is **Open-Closed Principle**. It is the idea that elements should be open for extension but not for modification, in that developers should be able to add new functionalities to a program but it shouldn't modify the current functionalities and current code.

`Chart.js` uses inheritance for many of its components, including but not limited to elements, scales, and controllers. By using inheritance, the library violates the Open-Closed Principle. This is because changing the parent components would also require changing the child components, thus making it NOT closed to modification. For instance, the `BarController` and `LineController` components both inherit from the `DatasetController` class. If you wanted to modify any of the functions in `DatasetController` (for example, wanting to modify the `scaleClip` function to accept a different kind of scale as a parameter), you'd also need to edit `BarController` and `LineController` to handle that different kind of scale. This same idea applies to the `Bar` and `Line` components, which inherit from the `Element` class.
...
Finally, this system follows the Liskov Substitution Principle - the principle that "objects should be replaceable with their subtypes without altering the correctness of the program." One instance of this are the [`Scale`](https://github.com/chartjs/Chart.js/blob/master/src/core/core.scale.js) and [`CategoryScale`](https://github.com/chartjs/Chart.js/blob/master/src/scales/scale.category.js) components. The subtype (`CategoryScale`) would be able to replace its parent (`Scale`). For instance, a new scale object of `CategoryScale` type is still able to use the `generateTickLabels()` and `calculateLabelRotation()` methods from the that it would've inherited from `Scale`. This same principle applies to the [`Element`](https://github.com/chartjs/Chart.js/blob/master/src/core/core.element.js) and [`LineElement`](https://github.com/chartjs/Chart.js/blob/master/src/elements/element.line.js) components. The subtype  `LineElement` would be able to replace its parent `Element`. For example, a new element object of `LineElement` type can still viably use the `getProps()` and `hasValue()` methods that it would've inherited from `Element`.
## System Improvement
...
| Refactoring: Fix Inline Variable | sdjfskd |
